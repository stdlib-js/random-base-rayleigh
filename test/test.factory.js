...
tape( 'attached to the returned function is the generator name', function test( t ) {
	...
	t.equal( rayleigh.NAME, 'rayleigh', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var rayleigh = factory();
	t.equal( typeof rayleigh.PRNG, 'function', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.equal( rayleigh.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.equal( isUint32Array( rayleigh.seed ), true, 'has property' );
	t.equal( rayleigh.seed[ 0 ], 12345, 'equal to provided seed' );

	rayleigh = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.equal( rayleigh.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var rayleigh = factory();
	t.equal( typeof rayleigh.seedLength, 'number', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.equal( rayleigh.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var rayleigh = factory();
	t.equal( isUint32Array( rayleigh.state ), true, 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.equal( rayleigh.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var rayleigh = factory();
	t.equal( typeof rayleigh.stateLength, 'number', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.equal( rayleigh.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var rayleigh = factory();
	t.equal( typeof rayleigh.byteLength, 'number', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.equal( rayleigh.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	rayleigh = factory();
	t.equal( typeof rayleigh.toJSON, 'function', 'has method' );
	...
	t.equal( typeof rayleigh.toJSON, 'function', 'has method' );
	t.equal( rayleigh.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.equal( rayleigh(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
