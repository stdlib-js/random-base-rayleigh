...
tape( 'the function throws an error if provided a scale parameter which is not a positive number (no options)', function test( t ) {
	var values;
	var i;
...
tape( 'the function throws an error if provided a scale parameter which is not a positive number (options)', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns a seeded pseudorandom number generator (integer seed)', function test( t ) {
	...
	rayleigh1 = factory( 3.5, {
		'seed': seed
	});
	rayleigh2 = factory( 3.5, {
		'seed': seed
	});

	t.notEqual( rayleigh1, rayleigh2, 'separate generators' );
...
tape( 'attached to the returned function is the generator name', function test( t ) {
	...
	t.strictEqual( rayleigh.NAME, 'rayleigh', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var rayleigh = factory();
	t.strictEqual( typeof rayleigh.PRNG, 'function', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( rayleigh.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( rayleigh.seed ), true, 'has property' );
	t.strictEqual( rayleigh.seed[ 0 ], 12345, 'equal to provided seed' );

	rayleigh = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( rayleigh.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var rayleigh = factory();
	t.strictEqual( typeof rayleigh.seedLength, 'number', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( rayleigh.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var rayleigh = factory();
	t.strictEqual( isUint32Array( rayleigh.state ), true, 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( rayleigh.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var rayleigh = factory();
	t.strictEqual( typeof rayleigh.stateLength, 'number', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( rayleigh.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var rayleigh = factory();
	t.strictEqual( typeof rayleigh.byteLength, 'number', 'has property' );

	rayleigh = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( rayleigh.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	rayleigh = factory();
	t.strictEqual( typeof rayleigh.toJSON, 'function', 'has method' );

	o = rayleigh.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, rayleigh.NAME, 'has property' );
	...
	t.strictEqual( typeof rayleigh.toJSON, 'function', 'has method' );
	t.strictEqual( rayleigh.toJSON(), null, 'returns expected value' );

	t.end();
});

tape( 'when called without arguments, the function returns a PRNG that returns `NaN` when provided a scale parameter equal to `NaN`', function test( t ) {
	var rayleigh;
	var r;
...
tape( 'when called without arguments, the function returns a PRNG that returns `NaN` when provided a scale parameter which is not a positive number', function test( t ) {
	var rayleigh;
	var r;
...
tape( 'the function returns a PRNG for generating random numbers from an Rayleigh distribution', opts, function test( t ) {
	var threshold;
	var count;
	var npass;
	var sigma;
	...
	function gof() {
		...
		rejected = 0;
		for ( i = 0; i < N; i++ ) {
			rayleigh = factory( sigma );
			t.ok( true, 'seed: '+rayleigh.seed );
			for ( j = 0; j < x.length; j++ ) {
				x[ j ] = rayleigh();
			}
			// Test using Kolmogorov-Smirnov goodness-of-fit test:
			pValue = kstest( x, 'rayleigh', sigma ).pValue;
	...
	function done( bool, rejected ) {
		t.ok( bool, 'null hypothesis (i.e., that numbers are drawn from Rayleigh('+sigma+') is rejected in less than '+(threshold*100)+'% of cases ('+rejected+' of '+N+'). Repeats: '+npass+' of '+count+'.' );
		t.end();
	}
});
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( rayleigh(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
